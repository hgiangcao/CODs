...
from  CODs_Training.loss_function.pixelwise_contrastive_loss import PixelwiseContrastiveLoss
import CODs_Training.loss_function.loss_composer as loss_composer

from CODs_Training.dense_correspondence_network import DenseCorrespondenceNetwork
from CODs_Training.ultils import *
from CODs_Training.CODsDataLoader_O2O import CODsDataset
...
class CODsTrainer(object):
    ...
    def setup(self):
        self.tensorboard  = self.setup_tensorboard("../CODs_Training/"+"tensorboard/"+self.config["setting"]+"_"+self.config["inputMode"]+"_"+config["model"]+str(config["descriptor_dimension"]), "comment")
    ...
    def setup_network(self):
        ...
        save_path = "../CODs_Training/"+self.config["saved_path"] + "/" + self.config["setting"]+"_"+self.config["inputMode"]+"_"+config["model"]+str(config["descriptor_dimension"])
    ...
    def save_network(self, iteration):
        print("Save models ", "{}/CODs_{}".format("../CODs_Training/"+self.config["saved_path"] + "/" + self.config["setting"]+"_"+self.config["inputMode"]+"_"+config["model"]+str(config["descriptor_dimension"]), iteration))
        torch.save(self.model.state_dict(), "{}/CODs_{}".format("../CODs_Training/"+self.config["saved_path"] + "/" + self.config["setting"]+"_"+self.config["inputMode"]+"_"+config["model"]+str(config["descriptor_dimension"]), iteration))
