...
from dataset.spartan_dataset_masked import SpartanDataset
...
import evaluation.plotting as dc_plotting

from correspondence_tools.correspondence_finder import random_sample_from_masked_image

from evaluation.utils import PandaDataFrameWrapper

# why CODs't we have scene_name
...
class DenseCorrespondenceEvaluation(object):
    ...
    def evaluate_network_across_objects(dcn=None, dataset=None, num_image_pairs=25):
        ...
        for i in xrange(num_image_pairs):
            ...
            # if the list is empty, CODs't bother +=ing it, just continue
    ...
    def plot_descriptor_colormaps(res_a, res_b, descriptor_image_stats=None,
        ...
        if plot_masked:
            ...
            if (mask_a_repeat.ndim==4):
                mask_a_repeat = mask_a_repeat[:,:,:,0]

            if (mask_b_repeat.ndim==4):
                mask_b_repeat = mask_b_repeat[:,:,:,0]
                
            print (mask_a_repeat.shape)
            print (res_a.shape)
            res_a_mask = mask_a_repeat * res_a
            print (res_a_mask.shape)
    ...
    def evaluate_network_qualitative(dcn, dataset, num_image_pairs=5, randomize=False,
                                     scene_type=None, isLoadTraniningSet = True):

        dcn.eval()
        # Train Data
        if isLoadTraniningSet:
    ...
    def run_evaluation_on_network(model_folder, num_image_pairs=100,
                                  ...
                                  iteration=None):
        """
        Runs all the quantitative evaluations on the models folder
