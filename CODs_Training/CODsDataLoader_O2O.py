...
from CODs_Training.DataGenerator_O2O import DataGenerator,loadAllModels
...
class CODsDataset(Dataset):
    def __init__(self,):
        self.dataGenerator = DataGenerator()
        self.models, self.colors = loadAllModels(path="../CODs_data/",loadFile=True)
        print (self.models[0].shape)
        self.dataGenerator.setOriginalModelObjects(self.models, self.colors)
        # self.models_ITRI, self.colors_ITRI = loadAllModels(nObject=13,path="dataCODs/ITRI_",loadFile=True)
    ...
    def __getitem__(self, idx):
        ...
        pathData =  path="../CODs_data/" # random.choice(["data2", "data"])
