...
from DON_Training.DataGenerator_O2O import DataGenerator,loadAllModels,convertToHeatmap,render,remove_values_from_list,processImage
...
def evaluate(savedModel,inputMode):
    ...
    DON = DensObjectNet(setting=config["inputMode"], pretrained=savedModel)
    nTry = 100
    nTestPoint = 1000
            ...
            for i in  tqdm(range(nTry)):
                ...
                while (nMatchPoint < nTestPoint):
                    imgA,depthA, imgB,depthB, rawData_1, rawData_2, matches_a, matches_b, _, _, _, _, _, _, nMatchPoint, nNoneMatchPoint=\
                                                        dataGenerator.generateRandomData(pathToScense="../DON_data/", matchType=0, sceneType=min(7,testCase[type_test]), augmentationType=augmentationType, debug=False, isEvaluate = True,isLoadUnseen=unseen,isLoadOrig = (testCase[type_test] == 8))
                ...
                descrtion1 = DON.dcn.forward_single_image_tensor(imgA.clone(),depthA.clone())
                descrtion2 = DON.dcn.forward_single_image_tensor(imgB.clone(),depthB.clone())
                ...
                for ith in range (nValidTestPoint):
                        px, py = u1[ith],v1[ith]
                        # print (px, py)
                        matchPoint, _ = DON.getBestMatchPointOnly((px, py), descrtion1, descrtion2, mask_2)
...
def evaluateSinglePair(savedModel, inputMode,sceneType = None,unseen = 0):
    ...
    while (nMatchPoint < nTestPoint):
        imgA, depthA, imgB, depthB, rawData_1, rawData_2, matches_a, matches_b, _, _, _, _, _, _, nMatchPoint, nNoneMatchPoint = \
            dataGenerator.generateRandomData(pathToScense="../DON_data/", matchType=0,
                                             sceneType= sceneType,
