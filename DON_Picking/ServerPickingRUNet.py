...
#from DON_Picking.VPG_net import RUNet,ObsTransformer
...
def get_args():
    ...
    parser.add_argument("--DON_pretrained", type=str, default="CODs_GraspNet_RGBD_Resnet8/DON_10001")
...
def train(opt):
    ...
    if (True):
        ...
        trainModel = RUNet(opt.DON_mode, 1,DON_pretrained=opt.DON_pretrained)
            ...
            for step2M in range(12):  # for each 4M timestep. Total 48M
                for epi in tqdm(range(nEpisode)):
                    ...
                    for currentStep in tqdm(range(num_local_steps)):
                        ...
                        lastDone = torch.FloatTensor(lastDone).cuda(0)

                        # append last state: old_log_policies || actions || values || states || obs  || rewards  || maskes || dones
