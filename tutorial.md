...
* CODs_training_tutorial: tutorial for training CODs.
* CODs_Picking_Real_tutorial: tutorial for test picking cluttered objects.
...
For training CODs, please refer to `notebooks/CODs_Training_tutorial.ipynb`.
...
After finish the training, there is a small test to verify that we succesfully trained the CODs. The program will load a pre-trained model and evaluate 1000 matching-point pairs from a random pair of images.
...
## CODs Evaluation
To evaluate CODs result in detail, run `CODs_Training/testCorrespondace_O2O.py` to visualize the finding matching point between 2 seleted pair of images. These two images should both contain some same objects (at least one).
The result should look similar as below:
