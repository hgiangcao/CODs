...
def get_args():
    ...
    parser.add_argument('--CODs_pretrained', type=str, default="CODs_GraspNet_RGBD_Resnet8/CODs_5001")
    ...
    parser.add_argument("--setting", type=str, default="Picking_CODs_GraspNet")
...
def train(opt):
    CODs_Mod = opt.CODs_mode
    CODs = DensObjectNet(setting="D", pretrained=opt.CODs_pretrained)
    ...
    for step2M in range(12):  # for each 4M timestep. Total 48M
        ...
        for epi in tqdm(range(nEpisode)):
            ...
            for currentStep in range(opt.num_local_steps):
                ...
                tensorDescription = CODs.getDescriptorTensor(tensorListRGB)
            ...
            tensorboard.add_scalar("DieReason/CODsE_FINISH", errorCode[CODsE_FINISH] / nTrajectory, totalTimeStep)
            tensorboard.add_scalar("DieReason/CODsE_CRASH", errorCode[CODsE_CRASH] / nTrajectory, totalTimeStep)
            tensorboard.add_scalar("DieReason/CODsE_EXCEEDED_MAX_ACTION", errorCode[CODsE_EXCEEDED_MAX_ACTION] / nTrajectory, totalTimeStep)
            tensorboard.add_scalar("DieReason/CODsE_OUT_AREA", errorCode[CODsE_OUT_AREA] / nTrajectory, totalTimeStep)
            ...
            for value, reward, CODse in stackedData:
                '''
                delta = reward  + opt.gamma * next_value.detach() * (1 - CODse) - value.detach()
                gae = delta + opt.gamma * opt.tau * (1 - CODse) * gae
                R.append(gae + value)
                next_value = value
