...
from ITRIP.assets import UR5_SCENE_FILE_PATH
from ITRIP.utils import center_scaling, encode_bin,\
...
#from ITRIP.objects import TextureRandomizer
from ITRIP.utils import decode_bin
...
class SimSuctionEnv(SuctionBaseEnv):
    def __init__(self, envIdx = 0, img_res=(256,256), headless=False, debug=False, exclude=[], isEvaluation = False) -> None:
        ...
        self.pr = PyRep()
        self.pr.launch(UR5_SCENE_FILE_PATH, headless=headless)
        #print (UR5_SCENE_FILE_PATH)
        self.pr.start()
    ...
    def reset(self,
        ...
        #num_objs = np.random.randint(3,11)
        ...
        for _ in np.arange(num_objs):
            obj = self.obj_loader.get_random_obj(exclude=exclude)
            #obj = self.obj_loader.getCylider()
        ...
        '''
        if randomize_tex:
            assert self.tex_randomizer is not None, "Did not supply texture dir."
            for obj in self.objects:
                if np.random.uniform() < randomize_tex_prob:
                    self.tex_randomizer.randomize(obj)
        '''
        if (len(self.objects) < num_objs):
            return self.reset(num_objs = num_objs)

        #try to freeze objects
        #for obj in self.objects:
    ...
    def _take_obs(self, cam: VisionSensor):
        ...
        return depth, rgb, valid_actions_enc
    '''

    def _take_obs(self, cam: VisionSensor, filterMask = None):
        cam.handle_explicitly()  # explicit handling for faster simulation
        ...
        if (filterMask is not None):
            valid_actions_decode = np.logical_and(valid_actions_decode,filterMask)
